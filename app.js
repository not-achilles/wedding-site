        ...
        function fireConfetti() {
            ...
            for (let i = 0; i < numParticles; i++) {
                particles.push({
                    x: startX,
                    y: startY,
                    vx: -3.5 + Math.random() * 7, // Wider horizontal spread
                    vy: -6 - Math.random() * 8,    // High-powered upward launch
                    g: 0.2 + Math.random() * 0.15, // Fast gravity fall
                    w: isMobile ? (8 + Math.random() * 6) : (12 + Math.random() * 10),
                    h: isMobile ? (10 + Math.random() * 8) : (16 + Math.random() * 14),
                    color: colors[Math.floor(Math.random() * colors.length)],
                    rotation: Math.random() * 360,
                    rotationSpeed: -2 + Math.random() * 4, // Faster spin
                    opacity: 1
                });
            }
            ...
            function loop() {
                ...
                for (let i = 0; i < particles.length; i++) {
                    ...
                    p.opacity -= 0.012; // Snappy celebratory fade (lasts ~1.5 seconds)
